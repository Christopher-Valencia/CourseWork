...
{
    // TODO implement
    double **tempAlloc=allocation;
    double *tempAvail=available;
    double **tempNeed=need;

    //modify temporary tables to assess Safe State
    for(int x=0; x<numOfAccounts; x++)
    {
        tempAlloc[customerNum][x]+=request[x];
        tempNeed[customerNum][x]-=request[x];
        tempAvail[x]-=request[x];
    }

    //step1
    bool isCustomerSafe[numOfCustomers];
    for(int x=0;  x<numOfCustomers; x++)
        isCustomerSafe[x]=false;
    //step 2
    for(int x=0; x<numOfCustomers; x++)
    {
        if (isCustomerSafe[x] == false)
        {
            for (int y = 0; y < numOfAccounts; y++)
            {
                if (tempNeed[x][y] <= tempAvail[y])
                    isCustomerSafe[x] = true;
                else
                {
                    isCustomerSafe[x] = false;
                    break;
...
{
    // TODO implement
    double total[numOfAccounts]={0.0};

    for(int x=0; x<numOfAccounts; x++)
    {
        for(int y=0; y<numOfCustomers; y++)
        {
            total[x]+=allocation[y][x];
        }
    }

    for(int x=0; x<numOfAccounts; x++)
        if(request[x]>total[x])
            return false;
...
bool borrow(int customerNum, double funds[])
{
    ...
    // TODO: complete
    if(ret && isSafeState(customerNum, funds))
    {
        for(int x=0; x<numOfAccounts; x++)
        {
            allocation[customerNum][x]+=funds[x];
            need[customerNum][x]-=funds[x];
            available[x]-=funds[x];
        }
    }
